...
public class AiBuilder {
        ...
        for (byte i = 0; i < layers; i++) { //Creates nodes for all the layers specified in "layers"
            ...
            for (int j = 0; j < n.length; j++) { //Creates the right amount of nodes for the number specified in "parameters"
                ...
                if (i > 0) { //Check whether the the node is in the input layer
                    float[] weights = new float[nodes.get(i).length]; //Create an array with the size of the amount of nodes in the previous layer
    ...
    private void backpropagate(int layer) {
        for (Node node: //Iterates through all nodes in the layer
             nodes.get(layer)) {
            float[] weights = node.getWeights(); //Weights connected to the node
        ...
        if (layer < nodes.size() - 1) { //Checks whether the output layer has been reached
    ...
    private void calculateCostForNodes(int layer) {
        for (Node node: //Iterates through all nodes of the previous layer
             this.nodes.get(layer - 1)) {
            float[] weights = node.getWeights(); //Weights of the node
            float sum = 0; //Sum of the cost
            for (int i = 0; i < nodes.get(layer).length; i++) { //Iterated through all nodes of the layer
                sum += nodes.get(layer)[i].getValue() * weights[i]; //Adds the cost of the node to the sum
            }
